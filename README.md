...
这个仓库不依赖外部大模型 API，可以直接运行脚本看到 Agent 的 tool 调用过程。核心 prompt、context 和 tool definitions 均保存在仓库中，便于评分时检查。
...
- `reports/综合实践（阶段1）-实验2-实验报告-中文版.docx`：已填写的中文实验报告。
...
这个 Agent 会先读取实验要求 context，再提取截止时间、交付物和评分项，然后生成代码仓库检查清单、报告结构建议和 AI 使用反思草稿。它使用的上下文集成方式包括：文件型 rubric context、系统 prompt、tool schema，以及 execution trace 中的中间结果。

## 工具列表

Agent 当前注册了 5 个 tool：
